...
- Check for go.mod, go.sum, Makefile, etc.
- Identify project type (Go module, web app, CLI tool, etc.)
- Scan overall project structure (src/, cmd/, internal/, etc.)
- Check if this is a new or existing project
- Look for existing documentation (README, docs/, etc.)
- Check for database configuration (config.yaml, .env)
- Identify static assets and template directories
